...
if primaryGene == None and ncbi_gi != None:
   #primaryGene = efpDb.ncbiToGeneId(ncbi_gi)
   if primaryGene == None:
      primaryGene = ncbi_gi
...
defaultImgFilename = "%s/%s_image.png" % (efpConfig.dataDir, dataSource) # Hack for species name change by Asher!
if mode == None:
...
###-----------------------------------"all" view-------------------------------------------------------------------------------------------------------------------------------
	...
	gene2.setQueryGene(secondaryGene)
	    ...
	    if (view.dbGroup == "AtTAX"):
                errorStr = 'The requested Primary gene / probeset ID "%s" cannot be found in %s datasource ' % (primaryGene, view.name)
	    else:
		errorStr = 'The requested Primary gene / probeset ID "%s" cannot be found in %s datasource ' % (primaryGene, view.dbGroup)
            ...
            error = 1
            if (view.dbGroup == "AtTAX"):
	        errorStr = 'The requested Secondary gene / probeset ID "%s" cannot be found in %s datasource <br>' % (secondaryGene, view.name)
	    else:
            ...
            maxSignalInDatasource = None
            if (gene1.probesetId != None):
		...
		if viewMaxSignal > threshold and maxSignalInDatasource != '':
                    useThresholdFlag = "on"
...
#    species_name = efpConfig.spec_names[efpConfig.species]
#    alertStr = "View <a href='../../expressolog_treeviewer/cgi-bin/expressolog_treeviewer.cgi?primaryGene=%s&species=%s&dataset=%s&checkedspecies=%s'>tree</a> with sequence and expression similarity, and alignment, for all orthologs for %s."%(gene1.getGeneId(),species_name,"Developmental",efpConfig.checkedspecies,gene1.getGeneId())
...
print '<body><form action="efpWeb.cgi" name="efpForm" method="POST" onSubmit="return checkAGIs(this,regId)">'
...
# added condition in case AGI exists in one At-TAX dataset but not other
if mode != None and error == 0 or ((len(imgFilename) != 0 and ( ('Tiling' in views) or ('ATH1' in views) ))):
