...
#from xml.sax import saxutils # outdated
...
testing_efp = open("output/testing_efppy.txt", "w")
...
def write_fasta(pop_sequences, agi, agi_seq):
    ...
    if temp_fasta != None:
        ...
        fasta_file.write(">%s\n%s\n"%(agi, agi_seq))
        for gene, sequence in pop_sequences.iteritems():
            fasta_file.write(">%s\n%s\n"%(gene, sequence))
        fasta_file.close()
    return temp_fasta
...
class View:
	...
	testing_efp.write("self.dbGroup in View class init = %s\n"%self.dbGroup)
        self.conn = None
    ...
    # modified Nov 4 2009 (HN): - Some sample signals in the Root DB are equal to 0 (no signal), and cause errors during calculations. 
    # Added condition to check if signal is zero. If signal/control != 0, signal = abs(math.log(signal/control)/math.log(2)), else signal = 0
    def getViewMaxSignal(self, gene, ratio, gene2=None):
        ...
        for group in self.groups:
            ...
            for tissue in group.tissues:
                ...
                if viewMaxSignal == 0:
                    if(abs(signal) > viewMaxSignal):
                        viewMaxSignal = abs(signal)
                else:
    ...
    def saveChart(self, filename, mode):
        ...
        if (mode == "Absolute"):
            ...
            ax1.set_ylabel("%s"%(efpConfig.Y_AXIS[self.database]), size='x-small')
        elif (mode == "Relative"):
            y1_max = 1.1*max(y)
            y1_min = 1.1*min(y)
            ...
            ax1.set_ylabel("difference in %s"%(efpConfig.Y_AXIS[self.database]), size='x-small')
    ...
    def getImageMap(self, mode, gene1, gene2, useT, threshold, datasource, grey_low, grey_stddev):
        ...
        for extra in self.extras:
            if extra.parameters == True:
                if useT == None:
                    thresholdSwitch = ""
                    out += '<area shape="polygon" coords="%s" title="%s" href="%s&modeInput=%s&primaryGene=%s&secondaryGene=%s&override=%s&threshold=%s&modeMask_low=%s&modeMask_stddev=%s">\n' % (extra.coords, extra.name, extra.link, mode, gene1.getGeneId(), gene2.getGeneId(), thresholdSwitch, threshold, grey_low, grey_stddev)
                else:
                    out += '<area shape="polygon" coords="%s" title="%s" href="%s&modeInput=%s&primaryGene=%s&secondaryGene=%s&override=%s&threshold=%s&modeMask_low=%s&modeMask_stddev=%s">\n' % (extra.coords, extra.name, extra.link, mode, gene1.getGeneId(), gene2.getGeneId(), useT, threshold, grey_low, grey_stddev)
            else:
		    ...
		    out += '<area shape="polygon" coords="%s" title="%s" href="%s">\n' % (extra.coords, extra.name, extra.link)                    
                ...
                # is a heatmap button
                    ...
                    if gene1.getGeneId() in geneList:
                        extra.button = 1
                    if gene2.getGeneId() in geneList and mode == "Compare":
                        extra.button = 2
                    if gene1.getGeneId() in geneList and gene2.getGeneId() in geneList and mode == "Compare":
                        extra.button = 3
                    # draw button img map
                if extra.button == 1:
    ...
    def renderComparison(self, gene1, gene2, threshold=0.0):
        ...
        if threshold >= efpConfig.minThreshold_Compare:
            ...
            max = efpConfig.minThreshold_Compare
    ...
    def renderAbsolute(self, gene, threshold=0.0, grey_mask=False):
        ...
        for group in self.groups:
            for tissue in group.tissues:
                ...
                (signal,stddev) = tissue.getMeanSignal(gene)
		#testing_efp.write("signal before intensity = %s\n"%signal)
                if signal != 0:
    ...
    def drawImage(self, mode, maxSignalInDatasource, viewMaxSignal1, viewMaxSignal2, gene1, gene2, img):
        ...
        if(gene1.getLookup() != None):
            draw.text(efpConfig.GENE_ORTHO1_POS, gene1.getLookup(), font=font, fill=color)
        if mode == 'Compare':
	...
	testing_efp.write("maxSignalInDatasource = %s\nviewMaxSignal1 = %s\nviewMaxSignal2 = %s\n"%(maxSignalInDatasource, viewMaxSignal1, viewMaxSignal2))
            ...
            for scale_param in efpConfig.GRAPH_SCALE_UNIT[datasource]:
                if signal <= scale_param[0]:  # if signal is within range draw bar
                    self.drawLine(img, signal, scale_param[1], displaceX, 
                                  ...
                                  bottom, color)
                    testing_efp.write("signal = %s\nscale_param[1] = %s\ndisplaceX = %s\n"%(signal, scale_param[1], displaceX))
                    testing_efp.write("displayY = %s\n"%(displaceY - height *(efpConfig.GRAPH_SCALE_UNIT[datasource].index(scale_param)+1)))
		    testing_efp.write("bottom = %s\tcolor = %s\n"%(bottom, color))
    ...
    def getTissueSignal(self, gene, sample, control = 0):
        ...
        testing_efp.write("SELECT data_signal FROM sample_data WHERE (data_probeset_id=%s OR data_probeset_id=%s) AND data_bot_id=%s"%(gene.getProbeSetId(), gene.getGeneId(), sampleId))
        cursor.execute("SELECT data_signal FROM sample_data \
                        WHERE (data_probeset_id=%s OR data_probeset_id=%s) \
                        AND data_bot_id=%s", (gene.getProbeSetId(), gene.getGeneId(), sampleId))
        row = cursor.fetchone()
        signal = None
        if row != None:
            signal = row[0]
    ...
    def getMaxInDatasource(self, gene):
        ...
        overallMax = 0
        maxDatasource = ''
	testing_efp.write("in getMaxInDatasource\n")
	testing_efp.write("self.dbGroup = %s\n"%self.dbGroup)
    ...
    def getMaxSignal(self, gene):
        ...
        cursor = self.conn.cursor()
	cursor.execute("SELECT MAX(data_signal) FROM sample_data WHERE (data_probeset_id=%s OR data_probeset_id=%s)", (gene.getProbeSetId(), gene.getGeneId()))
    ...
    def createGene(self, geneId):
	return efpDb.Gene(geneId, self.database)

    '''
    @name: alterGene
    @desc: Alters genes for maize_iplant and other maize views, to take splice variants into account
    '''
    def alterGene(self, gene):
        if gene.geneId is not None: 	 
            if (self.database == "maize_RMA_linear"):
                MaizeConvert1 = re.match("^GRMZM(2|5)G[0-9]{6}$", gene.geneId)
                MaizeConvert2 = re.match("^AC[0-9]{6}\.[0-9]{1}_FG[0-9]{3}$", gene.geneId)
                MaizeQueryCheck = re.match(".*_T[0-9]{1,2}$", gene.queryGene)
                MaizeSp1 = "_T01"
	        if ((MaizeQueryCheck is not None) and (gene.geneId != gene.queryGene)):
		    gene.geneId = gene.queryGene
                elif MaizeConvert1 is not None:
                    gene.geneId = gene.geneId + MaizeSp1
                if MaizeConvert2 is not None:
                    gene.geneId = (gene.geneId).replace("FG", "FGT")
            if (self.database == "maize_iplant" or self.database == "maize_leaf_gradient" or self.database == "maize_rice_comparison"):
                MaizeConvert3 = re.match("GRMZM2G[0-9]{6}_T[0-9]{1,2}", gene.geneId)
                MaizeConvert4 = re.match("^AC[0-9]{6}\.[0-9]{1}_FGT[0-9]{3}$", gene.geneId)
                if MaizeConvert3 is not None:
                    gene.geneId = re.sub("_T[0-9]{1,2}", "", gene.geneId)
                if MaizeConvert4 is not None:
                    gene.geneId = (gene.geneId).replace("FGT", "FG")
            if (efpConfig.species == "MAIZE"):
	        gene.probesetId = gene.geneId
        return gene

    def alterWebserviceGene(self, gene):
        if gene.geneId is not None:	 
            MaizeConvert1 = re.match("^GRMZM(2|5)G[0-9]{6}$", gene.geneId)
            MaizeConvert2 = re.match("^AC[0-9]{6}\.[0-9]{1}_FG[0-9]{3}$", gene.geneId)
            MaizeSp1 = "_T01"
            if MaizeConvert1 is not None:
                gene.webservice_gene = gene.geneId + MaizeSp1
            elif MaizeConvert2 is not None:
                gene.webservice_gene = (gene.geneId).replace("FG", "FGT")
            else:
                gene.webservice_gene = gene.geneId
        return gene
...
class SpecimenHandler(ContentHandler):
	...
	if name == 'view':
	    try: 
            ...
            self.sampleDict = {}
	    try:
           	#testing_efp.write("in try: self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), attrs.getValueByQName('dbGroup'), '%s/' + attrs.getValueByQName('img'))\n" % (viewClass, efpConfig.dataDir))
           	#testing_efp.write("viewClass = %s\n"%viewClass)
		#testing_efp.write("name = %s\n"%(attrs.getValueByQName('name')))
		#testing_efp.write("db = %s\n"%(attrs.getValueByQName('db')))
		#testing_efp.write("dbGroup = %s\n"%(attrs.getValueByQName('dbGroup')))
		#testing_efp.write("image = '%s/' + %s\n")%(efpConfig.dataDir, attrs.getValueByQName('img'))
           	exec "self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), attrs.getValueByQName('dbGroup'), '%s/' + attrs.getValueByQName('img'))" % (viewClass, efpConfig.dataDir)
		
	    except KeyError, e:
		#testing_efp.write("in except: self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), attrs.get('dbGroup'), '%s/' + attrs.getValueByQName('img'))" % (viewClass, efpConfig.dataDir))
	    ...
	    #testing_efp.write("dbGroup in SpecimenHandler = %s\n"%dbGroupName)
            #exec "self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), attrs.get('dbGroup'), '%s/' + attrs.getValueByQName('img'))" % (viewClass, efpConfig.dataDir)
            #testing_efp.write("self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), '%s', '%s/' + attrs.getValueByQName('img'))" % (viewClass, dbGroupName, efpConfig.dataDir))
            #exec "self.currentView = %s(attrs.getValueByQName('name'), attrs.getValueByQName('db'), '%s', '%s/' + attrs.getValueByQName('img'))" % (viewClass, dbGroupName, efpConfig.dataDir)
